...
test('сон через полночь сохраняется, редактируется и входит в выгрузку', async () => {
  ...
  try {
    const record = { sleptAt: '2026-09-23T23:40', wokeAt: '2026-09-24T08:10', note: 'Выспался', dream: 'Снился поезд\nи знакомый город' }
    let result = await request('/api/sleep', 'POST', record)
    assert.equal(result.status, 201)
    assert.equal(result.body.durationMinutes, 510)
    assert.equal(result.body.dream, record.dream)
    const id = result.body.id
    result = await request('/api/sleep', 'POST', { sleptAt: '2026-09-22T23:00', wokeAt: '2026-09-23T07:00' })
    assert.equal(result.body.dream, '')
    assert.equal(result.body.note, '')
    const plainId = result.body.id
    ...
    assert.equal(result.status, 400)
    result = await request('/api/sleep', 'POST', { ...record, dream: 'x'.repeat(10001) })
    assert.equal(result.status, 400)
    result = await request(`/api/sleep/${id}`, 'PUT', { ...record, wokeAt: '2026-09-24T09:10', note: 'После смены', dream: '  Снилось море  ' })
    assert.equal(result.body.durationMinutes, 570)
    assert.equal(result.body.dream, 'Снилось море')
    assert.equal((await request('/api/sleep')).body[0].note, 'После смены')
    const backup = await request('/api/export')
    assert.equal(backup.status, 200)
    assert.equal(backup.body.sleep_entries.length, 2)
    assert.equal(backup.body.sleep_entries.find((entry: { id: string }) => entry.id === id).dream, 'Снилось море')
    assert.equal((await request(`/api/sleep/${id}`, 'DELETE')).status, 204)
    assert.equal((await request(`/api/sleep/${plainId}`, 'DELETE')).status, 204)
